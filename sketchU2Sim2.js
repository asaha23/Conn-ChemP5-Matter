...
img1 = loadImage('Silicon-Dioxide.png');
}
...
function draw() {
   ...
   applyforceSio(0.001);
    ...
    function applyforceSio(mag) {
        for (var i = 0; i < particleSiO.length; i++) {
            for (var j = 0; j < particleSiO.length; j++) {
                var pvector = createVector((particleSiO[i].position.x),(particleSiO[i].position.y));
                var p2vector = createVector((particleSiO[j].position.x),(particleSiO[j].position.y));
                var distforce = dist(particleSiO[i].position.x, particleSiO[i].position.y, particleSiO[j].position.x, particleSiO[j].position.y);
                var diffvector = p5.Vector.sub(pvector, p2vector);
                diffvector.normalize();
                diffvector.mult(mag);
                if (distforce > 40) {
                    Matter.Body.applyForce(particleSiO[j], p2vector, diffvector);
                }
            }
        }
    }
...
function drawcircle() {
   ...
   //Drawing Silicon Dioxide
    stroke(255);
    strokeWeight(2);
    fill(255, 50);
   for (var i = 0; i < particleSiO.length;i++)
   {
    var Sio = particleSiO[i];
    var Siopos = Sio.position;
    var Sioangle = Sio.angle;
        
        push();
        translate(Siopos.x, Siopos.y);
        //rotate(Naangle);
...
function addMoleculesBtnSio(){
    addMoleculesSio(document.getElementById("moleculevalSio").innerHTML);
}

function addMoleculesSio(number)
{
     function particleSiO(x, y) {
        var params = {
            restitution: 1.0,
            friction: 0,
            offset: 0,
 ...
 for (var i= 0,x=200,y = 100; i< number;i++,x+=40)
   {
   particleSiO.push(new particleSiO(x,y));
   if (particleSiO.length < 3)
    {
    particleSiO[i].isSleeping = true;
    console.log("working");
    }
   }
   ...
   //console.log(NaParticles.length + "  NA  " +ClParticles.length + "Cl");
    ...
    for (var i = (particleSiO.length - number)  ; i < particleSiO.length ;i++)
    {
    World.add(world,particleSiO[i]);
    }
...
function start(){
    ...
    if (particleSiO.length < 3)
    {
    particleSiO[i].isSleeping = false;
    console.log("working");
    }
    }
